...
import type { GlobeArc, FlowEntry, StreamEvent } from './types';

export function nowTimeStr(): string {
  return new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' });
}

export function formatTimestampToTimeStr(isoString: string | undefined): string {
  try {
...
export function alertTitle(data: { alert_type?: string }): string {
...
export function getGlobalArcs(): GlobeArc[] {
...
export function getSelectedFlowEndpoints(): Endpoints | null {
  ...
  if (state.selectedFlowId.startsWith('stream-')) {
    ...
    const arc = state.activeArcs?.find(
      a => a.id === state.selectedFlowId || (a.flows && a.flows.some(f => f.id === state.selectedFlowId))
    );
    if (arc) {
...
export function checkIsLocalIp(ip: string): boolean {
  if (!ip) return false;
...
export function checkIsLocalConnection(src: string, dst: string): boolean {
