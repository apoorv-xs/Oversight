...
import { trackFlow, prependToStream, showAnomalyToast, showSystemToast, addToNotificationHistory, clearDashboardUi, updatePanelDots } from './ui_v2.js';
import { addGlobeArc } from './globe_v2.js';
import { audio } from './audio_v2.js';
import { formatTimestampToTimeStr } from './utils_v2.js';

export function handleStartCapture(clearUi = true) {
    ...
    if (!state.radarSweepInterval) {
        ...
        state.radarSweepInterval = setInterval(() => {
            if (state.isCapturing && document.visibilityState === 'visible') {
                audio.playRadarSweep();
            }
        }, 4000);
    }
...
export function handleStopCapture(errorMsg = null) {
    ...
    clearDashboardUi();
    if (errorMsg) {
        showSystemToast('CAPTURE ENGINE FAILURE', errorMsg, true);
    }
}

export function toggleAiCapture() {
    const btnToggle = document.getElementById('btnToggleCapture');
    if (!btnToggle) return;
        ...
        fetch('/api/start-capture', {
            ...
            .then(data => {
                if (data.status === 'success' || (data.message && data.message.includes('already running'))) {
                    ...
                    if (data.warning) {
                        showSystemToast('PRIVILEGE WARNING', data.warning, false);
                    }
                } else {
                    setRetryState(btnToggle);
                    const errMsg = data.message || 'Failed to start threat capture engine.';
                    showSystemToast('CAPTURE ENGINE FAILURE', errMsg, true);
                }
            })
            .catch((err) => {
                setRetryState(btnToggle);
...
/** Resets the capture button to its error/retry state, then restores it after 3 s. */
function setRetryState(btnToggle) {
    btnToggle.textContent = 'Retry';
    setTimeout(() => { if (!state.isCapturing) btnToggle.textContent = 'Start Engine'; }, 3000);
}

import { heartbeatViz } from './heartbeat_v2.js';
import { confidenceViz } from './confidence_v2.js';
...
export function initSocketEvents() {
    ...
    state.socket.on('packet', (data) => {
        if (state.lockedOnIp && data.src !== state.lockedOnIp && data.dst !== state.lockedOnIp) return;
        
        // Feed raw packets into the state tracker for instant UI representation
        // We purposefully omit prependToStream here so the left panel doesn't scroll at 1,000 lines per second
    ...
    state.socket.on('normal_flow', (data) => {
        if (state.lockedOnIp && data.src !== state.lockedOnIp && data.dst !== state.lockedOnIp) return;
...
export async function updateBackgroundStats() {
    ...
    try {
        ...
        if (data.status === 'success' && data.data) {
            ...
            // Sync capturing state from backend
            if (isCapturing !== state.isCapturing) {
                if (isCapturing) {
                    handleStartCapture(true);
                } else {
                    handleStopCapture(captureErr);
                }
            }

            // Bottom bar: traffic telemetry
            ...
            if (barEngine) {
                ...
                if (isCapturing) {
                    barEngine.classList.add('engine-active');
                } else {
                    barEngine.classList.remove('engine-active');
                }
            }

            // Sync recent anomalies into streamEvents to support anomalies/safe filtering
            if (isCapturing && stats && stats.recent_anomalies && stats.recent_anomalies.length > 0) {
                ...
                for (const anomaly of stats.recent_anomalies) {
                    const exists = state.streamEvents.some(e => 
                        e.src === anomaly.src && 
                        e.dst === anomaly.dst && 
                        e.dport === anomaly.dport && 
                    ...
                    if (!exists) {
                        state.streamEvents.push({
                            ...
                            src: anomaly.src,
                            src_host: anomaly.src_host,
                            dst: anomaly.dst,
                            dst_host: anomaly.dst_host,
                            dport: anomaly.dport,
                            isAnomaly: true,
                            isSafe: false,
                            time: formatTimestampToTimeStr(anomaly.timestamp),
                            timestamp: anomaly.timestamp,
                            reconstruction_error: anomaly.reconstruction_error || null,
                            raw_features: anomaly.raw_features || null
                        });
                        updated = true;
                    }
                }
                if (updated) {
                    // Sort state.streamEvents in reverse chronological order
                    state.streamEvents.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
                    // Keep hard limit at 100
                    if (state.streamEvents.length > 100) {
                        state.streamEvents = state.streamEvents.slice(0, 100);
                    }
                    // Trigger stream and top users re-render
                    import('./ui_v2.js').then(module => module.renderStreamList());
                }
            }
        }
    } catch (e) {
    ...
    try {
        ...
        if (data2.status === 'success' && data2.data) {
            ...
            if (barThreshold) barThreshold.innerText = adaptive.model_threshold ? parseFloat(adaptive.model_threshold).toFixed(4) : '--';
            if (barBaseline) barBaseline.innerText = adaptive.total_baseline ? adaptive.total_baseline.toLocaleString() : '--';
            
            // Progress = new flows since last retrain / retrain threshold
            const pct = adaptive.retrain_threshold > 0
                ? Math.min((adaptive.new_since_retrain / adaptive.retrain_threshold) * 100, 100)
                : 0;
...
export function updateFlowTimeout(seconds) {
    fetch('/api/update-timeout', {
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                console.error('Failed to update timeout:', data.message);
                showSystemToast('TIMEOUT SYNC ERROR', data.message || 'Failed to sync timeout with server.', true);
            }
        })
        .catch(err => {
            console.error('Error updating flow timeout:', err);
            showSystemToast('TIMEOUT SYNC FAILURE', 'Connection error while saving timeout value.', true);
        });
}
